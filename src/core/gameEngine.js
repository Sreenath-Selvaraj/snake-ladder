...
class GameEngine {
  constructor(players, dice, board, startIndex) {
  ...
  startGame() {
    while (!this.isGameOver) {
      ...
      const { extraTurn, mustRollOne, canMove } = this.ruleEngine.evaluateRules(player, roll);
      if (!extraTurn) {
        this.turnManager.nextTurn();
      }
      let newPosition = player.position + roll;
      newPosition = this.board.applySnakesAndLadders(newPosition);
      if (!player.hasStarted && mustRollOne) {
